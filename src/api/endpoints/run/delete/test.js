...
(() => {
  ...
  describe(model.title, () => {
    ...
    it('return 404 HTTP status code and "RUN_NOT_FOUND" response code when regular user tries to delete non-existing run', async () => {
      await users.create(sampleUser);

      const req = httpMocks.createRequest(modelFunctions.clone(sampleRequestOptions1, {
          body: {
            runId: "5d40e8dc54a81719ec938b33"
          }
        })),
        res = httpMocks.createResponse();
    ...
    it('return 403 HTTP status code and "NOT_ALLOWED" response code when regular user tries to delete other user\'s run', async () => {
      await users.create(sampleUser);
      let firstRun = (await (runs.create(sampleRun1DB))).toObject();
      await reports.create(sampleReport1);
    ...
    it('return 403 HTTP status code and "NOT_ALLOWED" response code when manager tries to delete other user\'s run', async () => {
      await users.create(sampleUser);
      let firstRun = (await (runs.create(sampleRun1DB))).toObject();
      await reports.create(sampleReport1);
    ...
    it('return 401 HTTP status code and "INVALID_SESSION" response code when passing wrong session token', async () => {
      ...
      assert.notEqual(dbRun, null, "run in db");
      assert.notEqual(report, null, "report in db");
      assert.equal(JSON.parse(res._getData()).response.session, undefined, "don't return session");
    });
