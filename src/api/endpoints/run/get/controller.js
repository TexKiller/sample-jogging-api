    ...
    if (loggedUser) {
      if (req.body.userName && req.body.userName !== loggedUser.name && loggedUser.role !== "admin") {
        return res.status(403).json({
          response: {
            status: "error",
            message: "The logged user is not allowed to list other user's runs",
            code: "NOT_ALLOWED",
      ...
      let total;
      try {
        total = (await (runs.aggregate(totalAggregation)))[0];
      } catch (e) {
        return res.status(403).json({
          response: {
            status: "error",
      ...
      if (total && total.count > aggregation[1].$skip) {
        ...
        result = (await (runs.aggregate(pageAggregation))).map(run => {
          let ret = runModel.db2api(modelFunctions.db2api(runModel.mongooseSchema)(new runs(run))).toObject();
          if (ret.path && ret.path.length === 0) {
            delete ret.path;
          }
          ret.time = new Date(ret.time);
          return ret;
        });
      } else {
        result = [];
